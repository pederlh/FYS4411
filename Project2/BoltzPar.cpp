...
BoltzPar::BoltzPar(int num_particles,int dimentions, double eta, int MC, int type_sampling, int interaction, double omega, int thread_ID)
{
   ...
   if (type_sampling == 0)
   {
       MetropolisMethod = &BoltzPar::Metropolis;
   }

   if (type_sampling == 1)
   {
       MetropolisMethod = &BoltzPar::Metropolis_Hastings;
...
double BoltzPar::MonteCarlo()
{
    ...
    if (it_num == its-1){DeltaE_.save("EnergySamples_ID_"+to_string(thread_ID_)+".txt", raw_ascii);}
    return energy;
}

void BoltzPar::Derivate_wavefunction()
{
    Q_factor(r_old_);
...
double BoltzPar::WaveFunction(mat r)
{
    Q_factor(r);
    double term1 = 0.0;
    double term2 = 1.0;
...
void BoltzPar::Metropolis_Hastings()
{
...
mat BoltzPar::QuantumForce(mat r)
{
...
double BoltzPar::GreensFunction(int idx)
{
    double G = 0.0;
    for (int d = 0; d < D_; d++){
...
void BoltzPar::SGD()
{
    ...
    for (int i = 0; i < its;  i++){
        ...
        # pragma omp master
        {
            cout << "Energy = " << setprecision(15) << Energy << "Thread ID = " << thread_ID_ << endl;
        }
        */
        cout << "Energy = " << setprecision(15) << Energy << "Thread ID = " << thread_ID_ << endl;
        ...
        if (i == its-1){
            ...
            cout << "Im "<<thread_ID_<< " and my b JOINED is " << b_joined << endl;
            ...
            cout << "Im "<<thread_ID_<< " and my b is " << b_ << endl;
            MC_ *=pow(2,4);
