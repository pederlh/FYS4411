...
vec alphas_,energies_, variances, E_L_to_file_, alpha_list_times_;
bool OBD_check_;
random_device rd_;
...
void Gradient_descent(vec shared_alphas);

// During GD process: Interacting or noninteracting case
void MonteCarlo_GD_noninteracting(vec values, double alpha);

// Final run after GD process: Interacting or noninteracting case
void MonteCarlo_optval_noninteracting(double alpha, vec energies);
void MonteCarlo_optval_interacting(double alpha, vec energies);
